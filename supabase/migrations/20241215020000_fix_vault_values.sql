-- Recalculate vault values for all artworks
CREATE OR REPLACE FUNCTION recalculate_artwork_vault_values()
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
    FOR v_artwork IN SELECT id FROM public.artworks LOOP
        ...
        SET accumulated_value = v_total_sln
        WHERE artwork_id = v_artwork.id;
    END LOOP;
END;
$$;
...
SELECT recalculate_artwork_vault_values();
