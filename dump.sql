...
CREATE OR REPLACE FUNCTION "public"."cast_vote"("p_artwork_id" "uuid", "p_pack_id" "uuid", "p_value" integer) RETURNS "uuid"
...
BEGIN
    ...
    SET
        total_votes = COALESCE(vault_states.total_votes, 0) + p_value,
        last_vote_at = now(),
        updated_at = now();
...
ALTER TABLE ONLY "public"."vault_states"
    ADD CONSTRAINT "vault_states_artwork_id_key" UNIQUE ("artwork_id");



ALTER TABLE ONLY "public"."vault_states"
    ADD CONSTRAINT "vault_states_pkey" PRIMARY KEY ("id");
...
CREATE OR REPLACE TRIGGER "update_vault_states_updated_at" BEFORE UPDATE ON "public"."vault_states" FOR EACH ROW EXECUTE FUNCTION "public"."update_updated_at_column"();
...
CREATE POLICY "Enable insert for authenticated users only" ON "public"."vault_states" FOR INSERT WITH CHECK (("auth"."uid"() IS NOT NULL));
...
CREATE POLICY "Enable read access for all users" ON "public"."vault_states" FOR SELECT USING (true);
...
CREATE POLICY "Enable update for authenticated users only" ON "public"."vault_states" FOR UPDATE USING (("auth"."uid"() IS NOT NULL));
